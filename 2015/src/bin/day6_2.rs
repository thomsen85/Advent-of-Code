...
fn apply_action_on_grid(action: Action, range: Range, grid: &mut Vec<Vec<i32>>) {
    for y in range.f_y..=range.t_y {
        for x in range.f_x..=range.t_x {
            match action {
                Action::TurnOff => grid[y][x] = 0.max(grid[y][x] - 1),
...
fn get_lights_on(input: Vec<String>) -> i32 {
    ...
    grid.iter()
        .flatten() // Flatten the 2D array to an iterator of lights
        .sum()
}
...
mod tests {
    ...
    #[test]
    fn example1() {
        let test = vec!["turn on 0,0 through 0,0".to_owned()];
        assert_eq!(get_lights_on(test), 1);
    }

    #[test]
    fn example2() {
        let test = vec!["toggle 0,0 through 999,999".to_owned()];
        assert_eq!(get_lights_on(test), 2000000);
    }
}
